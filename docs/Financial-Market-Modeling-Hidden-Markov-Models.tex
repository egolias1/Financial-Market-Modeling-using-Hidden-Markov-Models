...
\begin{tikzpicture}[node distance=2cm, auto]
    % Initialization
    \node[rectangle, draw] (init) {Initialize\\model parameters};
    \node[rectangle, draw, below=of init] (forward) {Forward\\algorithm};
    \node[rectangle, draw, below=of forward] (backward) {Backward\\algorithm};
    \node[rectangle, draw, below=of backward] (update) {Update\\model parameters};
    \node[rectangle, draw, below=of update] (converge) {Check\\convergence};
    \node[rectangle, draw, below=of converge] (output) {Output\\final parameters};
  
    % Arrows
    \draw[->] (init) -- (forward);
    \draw[->] (forward) -- (backward);
    \draw[->] (backward) -- (update);
    \draw[->] (update) -- (converge);
    \draw[->] (converge) -- node[align=center] {Converged?\\No} (output);
    \draw[->] (converge.east) -- ++(1.5cm,0cm) |- node[align=center] {Converged?\\Yes} (output.east);
    \draw[->] (converge.west) -- ++(-1.5cm,0cm) |- (forward.west);
\end{tikzpicture}
...
\begin{figure}
    ...
    \begin{tikzpicture}[
      node distance=1.5cm,
      auto,
      viterbi/.style={rectangle, draw, minimum height=0.7cm, minimum width=1cm, text centered, fill=blue!20},
      observation/.style={circle, draw, minimum size=0.5cm, text centered, fill=green!20},
      arrow/.style={thick,->,>=stealth},
    ]
    
    % Nodes
    \node[observation] (o1) {O1};
    \node[observation, right=0.8cm of o1] (o2) {O2};
    \node[observation, right=0.8cm of o2] (o3) {O3};
    
    \node[viterbi, below=0.8cm of o1] (s11) {S1};
    \node[viterbi, right=0.5cm of s11] (s21) {S2};
    \node[viterbi, right=0.5cm of s21] (s31) {S3};
    
    \node[viterbi, below=0.8cm of o2] (s12) {S1};
    \node[viterbi, right=0.5cm of s12] (s22) {S2};
    \node[viterbi, right=0.5cm of s22] (s32) {S3};
    
    \node[viterbi, below=0.8cm of o3] (s13) {S1};
    \node[viterbi, right=0.5cm of s13] (s23) {S2};
    \node[viterbi, right=0.5cm of s23] (s33) {S3};
    ...
    % Labels
    \node[above=0.1cm of o1] {Observations};
    \node[above=0.1cm of o2] {Observations};
    \node[above=0.1cm of o3] {Observations};
    
    \node[left=0.5cm of s11] {States};
    \node[left=0.5cm of s12] {States};
    \node[left=0.5cm of s13] {States};
    
    \end{tikzpicture}
    \caption{Viterbi algorithm illustration.}
    \label{fig:viterbi}
    \end{figure}
